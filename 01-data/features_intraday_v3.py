...
─────────────────────────────────────────────────────
Shifts temporels, forward-fill NWP, filtrage minimal.
...
def add_load_lags(df):
    log.info("  [1] Lags charge (J-1, J-7)")
...
def add_pv_lags(df):
    log.info("  [3] PV local (t-15min, t-1h, J-1, J-7) + PV remote (J-1, J-7)")
...
def add_nwp_features(df):
    log.info("  [7] NWP forward-fill (shift par horizon à l'entraînement)")
...
def add_interactions(df):
    log.info("  [8] Interactions météo récente")
...
def build_targets(df):
    log.info(f"  Cibles : target_1 à target_{HORIZON_MAX}")
...
def build_nwp_for_horizon(df, horizon):
    available = [c for c in NWP_COLS if c in df.columns]
...
def build_feature_matrix(df_oiken, df_meteo_real, df_meteo_pred):
    ...
    log.info(f"\n  Sans target_{HORIZON_MAX} : {n0 - df.height}")
    n1 = df.height
    df = df.filter(pl.col("load_lag_96").is_not_null())
    log.info(f"  Sans load_lag_96 : {n1 - df.height}")
    n2 = df.height
    df = df.filter(pl.col("load_lag_672").is_not_null())
    log.info(f"  Sans load_lag_672 : {n2 - df.height}")
    ...
    for c in feature_cols:
        ...
        if nc > 0:
            ...
            log.info(f"  Imputation: {c} ({nc} nulls)")
...
def run_feature_engineering_intraday():
    ...
    tr.write_parquet(OUTPUT_DIR / "train_intraday_v3.parquet")
    te.write_parquet(OUTPUT_DIR / "test_intraday_v3.parquet")
