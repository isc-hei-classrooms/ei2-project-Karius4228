"""
visualize_da.py — Visualisation Day-Ahead
"""
...
def plot_feature_importance(data, top_n=15):
    ...
    ax.barh(names, values, color="#3498db", edgecolor="#2c3e50", linewidth=0.5)
    ax.set_xlabel("Importance (gain)")
    ax.set_title("Top 15 Features — XGBoost Day-Ahead", fontsize=13, fontweight="bold")
    ax.grid(True, axis="x", alpha=0.3)
    plt.tight_layout()
    plt.savefig(FIGURES_DIR / "da_feature_importance.png", dpi=150, bbox_inches="tight")
    plt.close()
    print(f"  ✓ {FIGURES_DIR / 'da_feature_importance.png'}")
...
def plot_results_table(data):
    ...
    plt.tight_layout()
    plt.savefig(FIGURES_DIR / "da_mae_comparison.png", dpi=150, bbox_inches="tight")
    plt.close()
    print(f"  ✓ {FIGURES_DIR / 'da_mae_comparison.png'}")
...
def main():
    ...
    print("\n── Semaine hiver ──")
    plot_week(data, datetime(2025, 1, 13, tzinfo=timezone.utc),
              datetime(2025, 1, 20, tzinfo=timezone.utc),
              "Day-Ahead — Semaine hiver (13-19 jan 2025)", "da_predictions_winter.png")

    print("\n── Semaine été ──")
    plot_week(data, datetime(2025, 6, 2, tzinfo=timezone.utc),
              datetime(2025, 6, 9, tzinfo=timezone.utc),
              "Day-Ahead — Semaine été (2-8 juin 2025)", "da_predictions_summer.png")

    print("\n── Semaine printemps ──")
    plot_week(data, datetime(2025, 4, 7, tzinfo=timezone.utc),
              datetime(2025, 4, 14, tzinfo=timezone.utc),
              "Day-Ahead — Semaine printemps (7-13 avril 2025)", "da_predictions_spring.png")
